...
static bool rust_codegen_enabled()
{
    static bool const enabled = getenv("USE_RUST_CODEGEN") != nullptr;
    return enabled;
}
    ...
    StringView source_text, Realm& realm, StringView filename, size_t line_number_offset)
{
    ...
    if (!rust_codegen_enabled() && !compare_pipelines)
        return {};
...
GC::Ptr<Bytecode::Executable> compile_function(VM&, SharedFunctionInstanceData&)
{
    return nullptr;
}
