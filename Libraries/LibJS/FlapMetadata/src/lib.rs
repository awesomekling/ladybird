    ...
    for component in &specialization.components {
        ...
        for parameter in &component.parameters {
            ...
            match &parameter.constraint {
                SpecializationConstraint::Constant => name.push_str("Constant"),
                SpecializationConstraint::ConstantType(ty) => name.push_str(ty),
                SpecializationConstraint::ExactInteger(value) => {
                    name.push_str(if *value < 0 { "Negative" } else { "Value" });
                    name.push_str(&value.unsigned_abs().to_string());
                }
                SpecializationConstraint::SameOperand(relationship) => {
                    name.push_str("SameOperand");
                    name.push_str(&relationship.to_string());
                }
            }
        }
    }
...
pub fn parse_flap_metadata(source_name: &str, content: &str) -> Result<Vec<InstructionDefinition>, ParseError> {
    ...
    while line_index < lines.len() {
        ...
        while parenthesis_depth > 0 {
            ...
            declaration.push(' ');
            declaration.push_str(next_line.trim());
...
fn tokenize_specialization(source_name: &str, line: usize, source: &str) -> Result<Vec<String>, ParseError> {
    ...
    for character in source.chars() {
        if character.is_ascii_alphanumeric() || character == '_' || (character == '-' && current.is_empty()) {
...
impl SpecializationParser<'_> {
    ...
    fn parse_constraint(&mut self) -> Result<SpecializationConstraint, ParseError> {
        let spelling = self
            ...
            .to_string();
        self.index += 1;
        if spelling == "Operand" && self.consume("'") {
            let relation = self
                .current()
                .and_then(|token| token.parse::<u32>().ok())
                .ok_or_else(|| self.error("expected same-operand relationship number"))?;
            self.index += 1;
            return Ok(SpecializationConstraint::SameOperand(relation));
        }
        if spelling == "Constant" {
            return Ok(SpecializationConstraint::Constant);
        }
        if matches!(
            spelling.as_str(),
            "Int32" | "Double" | "String" | "Boolean" | "Null" | "Undefined"
        ) {
            return Ok(SpecializationConstraint::ConstantType(spelling));
        }
        if let Ok(value) = spelling.parse::<i32>() {
            return Ok(SpecializationConstraint::ExactInteger(value));
        }
        Err(self.error(format!("unknown specialization constraint '{spelling}'")))
    }
}
...
specialize Mov(dst: Operand'0) + Add(lhs: Operand'0, rhs: Constant)
    + JumpIf(condition: 0) as HotLoop;
specialize A() + B() + C() + D();
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            specializations[1].components[0].parameters[0].constraint,
            SpecializationConstraint::SameOperand(0)
        );
        assert_eq!(
            specializations[1].components[1].parameters[1].constraint,
            SpecializationConstraint::ConstantType("Int32".to_string())
        );
...
"#;
    ...
    fn rejects_invalid_specialization_declarations() {
        for (source, message) in [
            ...
            (
                "specialize Add(rhs: Operand');",
                "expected same-operand relationship number",
            ),
    ...
    fn parses_single_byte_mutations_without_panicking() {
        ...
        for index in 0..seed.len() {
            for replacement in [b' ', b'\n', b'<', b':', b'[', b']', b'0', b'a'] {
                ...
                let _ = parse_flap_metadata("mutated.def", source);
            }
        }
    }
    ...
    fn rejects_malformed_definitions() {
        for (source, message) in [
            ...
            let error = parse_flap_metadata("broken.def", source).unwrap_err();
            assert!(
