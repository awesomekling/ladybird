    ...
    pub fn assemble(&mut self) -> AssembledBytecode {
        ...
        for block in &mut self.basic_blocks {
            ...
            while index < block.instructions.len() {
                let Instruction::Mov { dst, src } = block.instructions[index].0 else {
                    index += 1;
                    continue;
                };
                if index + 1 < block.instructions.len()
                    && matches!(
                        block.instructions[index + 1].0,
                        Instruction::Mov {
                            dst: next_dst,
                            src: next_src,
                ...
                if index + 2 < block.instructions.len()
                    && matches!(block.instructions[index + 1].0, Instruction::Mov { .. })
                    && matches!(
                        block.instructions[index + 2].0,
                        Instruction::Mov {
                            dst: next_dst,
                            src: next_src,
                        } if next_dst == dst && next_src == src
                ...
                {
                    block.instructions.remove(index + 2);
                }
                index += 1;
            }
