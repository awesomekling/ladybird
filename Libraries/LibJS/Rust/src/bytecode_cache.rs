...
impl Encode for FunctionRecord<'_> {
    fn encode(&self, encoder: &mut Encoder) {
        ...
        SimpleParameterList(function_data).encode(encoder);
...
impl<'a> FunctionRecord<'a> {
    fn function_name(&self, function_data: &'a ast::FunctionData) -> Option<Utf16<'a>> {
        self.shared_data
            .name_override
            .as_deref()
            .or_else(|| function_data.name.as_ref().map(|name| name.name.as_slice()))
            .map(Utf16)
    }
}

struct SimpleParameterList<'a>(&'a ast::FunctionData);

impl Encode for SimpleParameterList<'_> {
    fn encode(&self, encoder: &mut Encoder) {
        let names = simple_parameter_names(self.0);
...
fn simple_parameter_names(function_data: &ast::FunctionData) -> Option<Vec<&[u16]>> {
    ...
    for parameter in &function_data.parameters {
        ...
        names.push(identifier.name.as_slice());
    }
    Some(names)
}
