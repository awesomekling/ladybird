...
unsafe fn string_from_raw<'a>(bytes: *const u8, len: usize) -> Option<&'a str> {
    unsafe {
        let bytes = bytes_from_raw(bytes, len)?;
...
pub unsafe extern "C" fn rust_content_blocker_create(rules: *const u8, rules_len: usize) -> *mut c_void {
    unsafe {
        abort_on_panic(|| {
            let Some(rules) = string_from_raw(rules, rules_len) else {
...
pub unsafe extern "C" fn rust_content_blocker_free(engine: *mut c_void) {
    unsafe {
        abort_on_panic(|| {
            if !engine.is_null() {
                drop(Box::from_raw(engine.cast::<adblock::engine::Engine>()));
            }
        });
    }
}
...
pub unsafe extern "C" fn rust_content_blocker_matches(
    ...
    unsafe {
        abort_on_panic(|| {
            ...
            let Some(url) = string_from_raw(url, url_len) else {
                return false;
            };
            let Some(source_url) = string_from_raw(source_url, source_url_len) else {
                return false;
            };
            let Some(request_type) = string_from_raw(request_type, request_type_len) else {
                return false;
            };

            let engine = &*engine.cast::<adblock::engine::Engine>();
...
pub unsafe extern "C" fn rust_content_blocker_cosmetic_css(
    ...
    unsafe {
        abort_on_panic(|| {
            ...
            let Some(classes) = string_from_raw(classes, classes_len) else {
                return string_to_ffi(String::new());
            };
            let Some(ids) = string_from_raw(ids, ids_len) else {
                return string_to_ffi(String::new());
            };
...
pub unsafe extern "C" fn rust_content_blocker_free_string(data: *mut u8, length: usize) {
    unsafe {
        abort_on_panic(|| {
            if !data.is_null() {
                drop(Box::from_raw(std::ptr::slice_from_raw_parts_mut(data, length)));
            }
        });
    }
}
