...
GC::Ptr<Layout::Node> HTMLInputElement::create_layout_node()
{
    ...
    if (computed_values()->appearance() == CSS::Appearance::None) {
        return Element::create_layout_node_for_display_type(document(), computed_values()->display(), *this);
    }
...
void HTMLInputElement::adjust_computed_style(CSS::ComputedProperties& style)
{
    ...
    if (is_single_line() && style.property(CSS::PropertyID::Height).has_auto()) {
        ...
        auto minimum_line_height = style.first_available_computed_font(document().font_computer())->pixel_size() * CSS::normal_line_height_scale;
...
void HTMLInputElement::update_text_input_shadow_tree()
{
    ...
    if (m_type == TypeAttributeState::Number) {
        ...
        if (auto const* values = computed_values(); values && values->appearance() == CSS::Appearance::Textfield) {
...
void HTMLInputElement::apply_presentational_hints(CSS::CascadedProperties& cascaded_properties) const
{
