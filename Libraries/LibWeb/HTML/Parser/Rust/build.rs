...
fn main() {
    ...
    let json_path = manifest_dir.join("../Entities.json");
    ...
    for i in 0..first_layer_count {
        ...
        first_available_index =
            queue_children(&node, &mut queue, &mut child_indexes, first_available_index);
    }
    ...
    // Generate DAFSA output file.
    let out_path = out_dir.join("named_character_references.rs");
    ...
    out.push_str(
        "            SecondCodepoint::CombiningLongVerticalLineOverlay => 0x20D2,\n",
    );
    ...
    for (first, second) in &index_to_codepoints {
        ...
        out.push_str(&format!(
            "    ({first:#06X}, SecondCodepoint::{variant}),\n"
        ));
    }
    out.push_str("];\n\n");
    ...
    for nd in &node_data {
        out.push_str(&format!(
            ...
            escape_byte(nd.character), nd.number, nd.end_of_word, nd.child_index, nd.children_len
        ));
    }
    out.push_str("];\n\n");
    ...
    fn get_unique_index(&self, word: &str) -> Option<usize> {
        ...
        for c in word.bytes() {
            let next = {
                ...
                if node.children[c as usize].is_none() {
                    return Option::None;
                }
                ...
                Rc::clone(node.children[c as usize].as_ref().unwrap())
            };
...
fn queue_children(
    ...
    for c in 0..128u8 {
        if let Some(child) = &node_ref.children[c as usize] {
            ...
            if !child_indexes.contains_key(&key) {
                let num_children = child.borrow().num_direct_children();
                if num_children > 0 {
                    child_indexes.insert(key, current);
