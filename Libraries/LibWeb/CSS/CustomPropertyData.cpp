...
RefPtr<CustomPropertyData const> CustomPropertyData::inheritable(DOM::Document const& document) const
{
    ...
    // Registration generations are local to each document, so the cache has to
    // include the destination document identity as well. Otherwise a subtree
    // adopted into another document can incorrectly reuse a filtered result
