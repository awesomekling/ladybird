        ...
        for (auto const& property : declaration.properties()) {
            ...
            for_each_property_expanding_shorthands(property.property_id, property_value, [&](PropertyID longhand_id, StyleValue const& longhand_value) {
                ...
                if (longhand_value.is_revert()) {
                    ...
                    // Track the exact shadow-root scope that supplied this
                    // winning declaration. A constructable stylesheet can be
                    // adopted into multiple scopes at once, so the declaration
                    // object alone is not specific enough.
    ...
    for (auto const& animation_properties : animation_definitions) {
        ...
        auto resolve_keyframes = [&]() -> RefPtr<Animations::KeyframeEffect::KeyFrameSet const> {
            if (auto animation_name_source_shadow_root = cascaded_properties.property_source_shadow_root(PropertyID::AnimationName)) {
                // The winning animation-name declaration can come from a
                // shadow-root rule even when the animated element itself is
                // outside that subtree, most notably for :host(...) and
                // ::slotted(...). Resolve @keyframes in the declaration's
                // scope first so same-named document rules do not win.
