...
bool selector_may_match_light_dom_under_shadow_host(Selector const& selector)
{
    ...
    // A bare :host selector only targets the host itself, but once a shadow
    // rule keeps walking to another compound it can match light-DOM nodes in
    // the host tree instead of staying confined to the shadow subtree.
...
// If the leading compounds (everything except the rightmost) of `compound_selectors`
// produce a usable invalidation set, return an anchor rule built from them.
static Optional<AnchorInvalidationRule> try_build_anchor_for_leading_compounds(Vector<Selector::CompoundSelector> const& compound_selectors, StyleInvalidationData& throwaway_data)
{
    ...
    return AnchorInvalidationRule { move(anchor_set), move(anchor_selector) };
}
...
void extend_style_sheet_invalidation_set_with_style_rule(StyleSheetInvalidationSet& result, CSSStyleRule const& style_rule)
{
    ...
    for (auto const& selector : style_rule.absolutized_selectors()) {
        ...
        // The rightmost compound has no targetable properties on its own, but
        // we can still avoid a whole-subtree invalidation if the leading
        // compounds carry an anchor invalidation set that we can match against.
        if (is_pseudo_element_only_compound(rightmost)) {
            if (auto anchor = try_build_anchor_for_leading_compounds(compound_selectors, throwaway_data); anchor.has_value()) {
...
static bool element_matches_anchor_rule(DOM::Element const& element, Rule const& rule, GC::Ptr<DOM::Element const> shadow_host)
{
...
static void apply_trailing_universal_combinator(Selector::Combinator combinator, DOM::Element& anchor, DOM::Node& target_root)
{
    ...
    case Selector::Combinator::Column:
        // The column combinator relates a <col>/<colgroup> anchor to the table
        // cells in the represented column. We do not yet have a more precise
        // invalidation primitive for column membership, so keep the work
        // bounded to the current table instead of dropping the invalidation.
...
// Walk `root` once and, for each element, check the primary invalidation set
// plus every anchor rule in one pass. Doing this as a single traversal instead
// of one walk per rule kind keeps the dirtying cost proportional to the tree
// size rather than to tree_size * rule_count.
    ...
    root.for_each_in_inclusive_subtree_of_type<DOM::Element>([&](DOM::Element& element) {
        ...
        if (!element.needs_style_update()) {
            for (auto const& rule : result.pseudo_element_rules) {
                if (element_matches_anchor_rule(element, rule, shadow_host)) {
                    element.set_needs_style_update(true);
                    break;
                }
            }
        ...
        // Trailing-universal rules dirty *other* elements (siblings or children
        // of the anchor), so we still have to check them even if `element` is
...
void invalidate_root_for_style_sheet_change(DOM::Node& root, StyleSheetInvalidationSet const& result, DOM::StyleInvalidationReason reason, bool force_broad_invalidation)
{
    ...
    if (force_broad_invalidation || invalidation_set.needs_invalidate_whole_subtree()) {
        ...
        // Broad shadow-root stylesheet mutations still have to reach the host
        // subtree because selectors such as ::slotted(...) can target light-DOM
        // descendants outside the shadow tree itself.
    ...
    invalidate_elements_matching_invalidation_set_and_anchor_rules(
        root, result, shadow_host_for_targeted_shadow_root_invalidation(root));

    if (auto* shadow_root = as_if<DOM::ShadowRoot>(root)) {
        invalidate_assigned_elements_for_dirty_slots(*shadow_root);

        if (auto* host = shadow_root->host()) {
            // Slotted selectors never match the host itself, so a targeted
            // ::slotted(...) invalidation only needs to walk the current host's
            // light-DOM subtree. We can identify that case because it reaches
            // host-side nodes without ever setting may_match_shadow_host.
            //
            // :host combinators are different: they can escape to siblings or
            // other nodes rooted alongside the host, so they still need the
            // broader host-root walk below.
            if (result.may_match_light_dom_under_shadow_host && !result.may_match_shadow_host) {
                invalidate_elements_matching_invalidation_set_and_anchor_rules(*host, result, host);
            } else if (result.may_match_light_dom_under_shadow_host) {
                invalidate_elements_matching_invalidation_set_and_anchor_rules(host->root(), result, host);
                ...
                for (auto const& rule : result.pseudo_element_rules) {
                    if (element_matches_anchor_rule(*host, rule, host)) {
                        host_or_shadow_tree_needs_style_update = true;
                        break;
                    }
                }
                if (host_or_shadow_tree_needs_style_update) {
                    ...
                    // A targeted :host rule can change inherited values seen by
                    // shadow descendants even when no selector directly matches
                    // inside the shadow tree. Mark the shadow root dirty too so
                    // recursive style update descends into that subtree.
                    shadow_root->set_needs_style_update(true);
                }
                for (auto const& rule : result.trailing_universal_rules) {
                    // Host-anchored trailing-universal selectors such as
                    // :host > * and :host * target the host's light-DOM tree,
                    // not descendants inside the shadow root. Keep the host as
                    // the selector anchor and dirty host-side nodes from there.
                    if (element_matches_anchor_rule(*host, rule, host))
