...
Optional<Parser::PropertyAndValue> Parser::parse_css_value_for_properties(ReadonlySpan<PropertyID> property_ids, TokenStream<ComponentValue>& tokens, Optional<StringView> original_source_text)
{
...

    {
        ...
        if (auto rust_style_value = RustComponentValueParser::parse_style_value_for_property(
            ...
            auto materialize_rust_url_image = [&](Optional<URL> const& typed_url) -> RefPtr<ImageStyleValue const> {
                if (!typed_url.has_value() || typed_url->url().starts_with('#'))
                    return nullptr;
            ...
            case FFI::CssStyleValueKind::GeneratedValueList: {
                ...
                for (size_t i = 0; i < rust_style_value->generated_value_list_value_types.size(); ++i) {
                    ...
                    Optional<String> original_component_value_source;
                    if (original_source_text.has_value())
                        original_component_value_source = component_values_original_source_text(component_values);
                    Optional<StringView> original_component_value_source_view;
                    if (original_component_value_source.has_value())
                        original_component_value_source_view = original_component_value_source->bytes_as_string_view();

                    TokenStream value_tokens { component_values };
                    auto value = parse_value(rust_style_value->generated_value_list_value_types[i], value_tokens, original_component_value_source_view);
                    value_tokens.discard_whitespace();
                    if (!value)
                        break;
