...
void ConnectionFromClient::start_request(u64 request_id, ByteString method, URL::URL url, Vector<HTTP::Header> request_headers, ByteBuffer request_body, Optional<Core::AnonymousBuffer> request_body_buffer, HTTP::CacheMode cache_mode, HTTP::Cookie::IncludeCredentials include_credentials, Core::ProxyData proxy_data)
{
    dbgln_if(REQUESTSERVER_DEBUG, "RequestServer: start_request({}, {})", request_id, url);

    auto request = Request::fetch(request_id, m_disk_cache, cache_mode, *this, m_curl_multi, m_resolver, move(url), move(method), HTTP::HeaderList::create(move(request_headers)), move(request_body), move(request_body_buffer), include_credentials, m_alt_svc_cache_path, proxy_data);
    m_active_requests.set(request_id, move(request));
}

void ConnectionFromClient::start_revalidation_request(Badge<Request>, ByteString method, URL::URL url, NonnullRefPtr<HTTP::HeaderList> request_headers, ByteBuffer request_body, Optional<Core::AnonymousBuffer> request_body_buffer, HTTP::Cookie::IncludeCredentials include_credentials, Core::ProxyData proxy_data)
{
    ...
    auto request = Request::revalidate(request_id, m_disk_cache, *this, m_curl_multi, m_resolver, move(url), move(method), move(request_headers), move(request_body), move(request_body_buffer), include_credentials, m_alt_svc_cache_path, proxy_data);
