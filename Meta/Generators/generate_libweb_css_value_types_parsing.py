...
def generate_rust_parser_expression(out: TextIO, value_type_name: str, value_type_index: int) -> bool:
    if value_type_name == "<font-weight-absolute>":
        out.write(f"""    auto syntax_kind = RustComponentValueParser::parse_a_value_type({value_type_index}, tokens);
    switch (syntax_kind) {{
    case FFI::CssValueTypeSyntaxKind::FontWeightAbsoluteNormal:
        return parse_specific_keyword_value(tokens, Keyword::Normal);
    case FFI::CssValueTypeSyntaxKind::FontWeightAbsoluteBold:
        return parse_specific_keyword_value(tokens, Keyword::Bold);
    case FFI::CssValueTypeSyntaxKind::FontWeightAbsoluteNumber:
        return parse_number_value(tokens, {{ 1.0, 1000.0 }});
    default:
        return nullptr;
    }}
""")
    ...
    if value_type_name == "<symbol>":
        out.write(f"""    auto syntax_kind = RustComponentValueParser::parse_a_value_type({value_type_index}, tokens);
    switch (syntax_kind) {{
    case FFI::CssValueTypeSyntaxKind::SymbolString:
        return parse_string_value(tokens);
    case FFI::CssValueTypeSyntaxKind::SymbolCustomIdent:
...
""")
        if generate_rust_parser_expression(out, value_type_name, value_type_index):
            out.write("""}
""")
        else:
