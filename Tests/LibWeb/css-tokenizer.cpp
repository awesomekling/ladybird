...
{
    StringView backend = "rust"sv;
    StringView encoding = "utf-8"sv;
    StringView input_path;
    bool silent = false;

    Core::ArgsParser args_parser;
    args_parser.add_option(backend, "Tokenizer backend to use (rust)", "backend", 'b', "backend");
    ...
    if (backend != "rust"sv) {
        warnln("Unknown backend '{}'. Expected 'rust'.", backend);
        return 1;
    }

    auto file = TRY(Core::File::open(input_path, Core::File::OpenMode::Read));
    auto input = TRY(file->read_until_eof());
    auto tokens = Web::CSS::Parser::RustTokenizer::tokenize(input, encoding);
