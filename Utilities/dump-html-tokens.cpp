...
ErrorOr<int> ladybird_main(Main::Arguments arguments)
{
    ...
    StringView last_start_tag;

    Core::ArgsParser args_parser;
    args_parser.set_general_help("Tokenize HTML and dump tokens in a canonical format.");
    ...
    args_parser.add_option(last_start_tag, "Last emitted start tag name (for RCDATA/ScriptData end tag matching)", "last-start-tag", 't', "name");
    ...
    if (!initial_state_name.is_empty()) {
        // Map state name string to enum value.
        Optional<HTMLTokenizer::State> state;
#define __ENUMERATE_TOKENIZER_STATE(s) \
    if (initial_state_name == #s##sv)  \
        state = HTMLTokenizer::State::s;
...
#undef __ENUMERATE_TOKENIZER_STATE
        ...
        tokenizer.switch_to(state.value());
    }
